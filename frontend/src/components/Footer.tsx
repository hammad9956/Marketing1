...
function SocialIcon({ id, paintKey }: { id: SocialPlatform; paintKey: number }) {
  ...
  const igGradId = `footer-social-ig-${paintKey}`;
  switch (id) {
    case "linkedin":
      return (
      ...
      <div className="mx-auto grid max-w-screen-2xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="mt-3 max-w-sm text-sm leading-relaxed text-slate-400">
            Conversion-obsessed websites, SEO dominance, apps, automation, and elite remote
            talent—built to grow ambitious US businesses.
          </p>
        </div>
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm text-slate-300">
            ...
            <li>
              <Link href="/portfolio" className="transition hover:text-brand">
                Portfolio
              </Link>
            </li>
            <li>
              <Link href="/blogs" className="transition hover:text-brand">
                Blog
              </Link>
            </li>
            <li>
              ...
              {socialLinks.map((s, i) => (
                <li key={`${s.href}-${i}`}>
                  <a
                    ...
                    className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-slate-900/60 text-slate-300 transition hover:border-brand/50 hover:text-brand"
                    aria-label={s.label}
                  >
                    <SocialIcon id={s.id} paintKey={i} />
                  </a>
                </li>
              ))}
            </ul>
